...
export function VideoPlayer({ videoData, isPlaying }: VideoPlayerProps) {
  ...
  return (
    <div style={{ width: VIDEO.width, height: VIDEO.height, position: 'relative', background: VIDEO.colors.bg, overflow: 'hidden' }}>
      {/* Thumbnail */}
      <img
        src={`/thumbnails/${videoData.thumbnail}`}
        alt=""
        style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: thumbnailOpacity, transition: 'opacity 0.1s' }}
      />

      {/* Fade to black */}
      {fadeToBlackOpacity > 0 && (
        <div style={{ position: 'absolute', inset: 0, background: '#0A0A0A', opacity: fadeToBlackOpacity }} />
      )}

      {/* Text */}
      {activeFrame && thumbnailOpacity < 0.01 && (
        <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: justify, paddingLeft: padLeft, paddingRight: padRight, paddingTop: 80, paddingBottom: 80, boxSizing: 'border-box' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 2, background: '#1A1A1A' }}>
        <div style={{ height: '100%', width: `${(currentSec / VIDEO.duration) * 100}%`, background: VIDEO.colors.accent }} />
      </div>

      {/* Timer */}
      <div style={{ position: 'absolute', bottom: 10, right: 12, fontFamily: VIDEO.fonts.mono, fontSize: 18, color: '#2D2D2D', letterSpacing: '0.1em' }}>
        {mm}:{ss}
      </div>
    </div>
  );

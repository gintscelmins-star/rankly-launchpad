...
export function ProfileGenerator() {
  ...
  const sectionHeader = (title: string, sub: string) => (
    <div style={{ marginBottom: 28 }}>
      <div style={{ fontSize: 11, color: '#4A4A4A', letterSpacing: '0.25em', marginBottom: 6 }}>{title}</div>
    ...
    <div style={{ background: '#1A1A1A', minHeight: '100vh', padding: 40, fontFamily: MONO, color: '#E8E8E8' }}>
        ...
        {PROFILE_VARIANTS.map((v, i) => {
          ...
          return (
            <div key={v.id} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {/* Preview apaļš */}
              <div style={{ width: 200, height: 200, borderRadius: '50%', overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
                <div style={{ width: w, height: h, transform: `scale(${scale})`, transformOrigin: 'top left' }}>
                  <ProfileCard variant={v} divRef={el => { profileRefs.current[i] = el; }} />
                </div>
              </div>
              <div style={{ fontSize: 11, color: '#6B6B6B' }}>{v.name}</div>
              <button
                onClick={async () => { const el = profileRefs.current[i]; if (el) await exportEl(el, `rankly-profile-${['a','b','c'][i]}.png`, w, h); }}
                style={{ background: 'none', border: '1px solid #2D2D2D', color: '#6B6B6B', padding: '7px 14px', fontFamily: MONO, fontSize: 11, cursor: 'pointer', textAlign: 'left' }}
              >
                ↓ PNG (800×800)
              </button>
            </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 56 }}>
        {FB_COVER_VARIANTS.map((v, i) => {
          ...
          return (
            <div key={v.id} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {/* Preview */}
              <div style={{ width: 410, height: previewH, overflow: 'hidden', position: 'relative' }}>
                <div style={{ width: w, height: h, transform: `scale(${scale})`, transformOrigin: 'top left' }}>
                  <FBCoverCard layout={v.layout} divRef={el => { coverRefs.current[i] = el; }} />
                </div>
                {/* Safe zone indikatori */}
                <div style={{ position: 'absolute', top: 0, left: 20, bottom: 0, width: 1, background: 'rgba(204,0,0,0.3)', pointerEvents: 'none' }} />
                <div style={{ position: 'absolute', top: 0, right: 20, bottom: 0, width: 1, background: 'rgba(204,0,0,0.3)', pointerEvents: 'none' }} />
              </div>
              <div style={{ fontSize: 11, color: '#6B6B6B' }}>{v.name}</div>
              <button
                onClick={async () => { const el = coverRefs.current[i]; if (el) await exportEl(el, `rankly-fb-cover-${['a','b','c'][i]}.png`, w, h); }}
                style={{ background: 'none', border: '1px solid #2D2D2D', color: '#6B6B6B', padding: '7px 14px', fontFamily: MONO, fontSize: 11, cursor: 'pointer', textAlign: 'left', width: 200 }}
              >
                ↓ PNG (1640×624)
              </button>
            </div>
      ...
      <div style={{ display: 'flex', gap: 24 }}>
        {IG_HIGHLIGHTS.map((hl, i) => {
          ...
          const scale = 120 / w;
          return (
            <div key={hl.id} style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center' }}>
              {/* Preview apaļš */}
              <div style={{ width: 120, height: 120, borderRadius: '50%', overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
                <div style={{ width: w, height: h, transform: `scale(${scale})`, transformOrigin: 'top left' }}>
                  <HighlightCard item={hl} divRef={el => { hlRefs.current[i] = el; }} />
                </div>
              </div>
              <div style={{ fontSize: 10, color: '#6B6B6B', textAlign: 'center' }}>{hl.name}</div>
              <button
                onClick={async () => { const el = hlRefs.current[i]; if (el) await exportEl(el, `rankly-ig-highlight-${hl.category}.png`, w, h); }}
                style={{ background: 'none', border: '1px solid #2D2D2D', color: '#6B6B6B', padding: '6px 10px', fontFamily: MONO, fontSize: 10, cursor: 'pointer' }}
              >
                ↓ PNG
              </button>
            </div>
